...
if ! command -v claude >/dev/null 2>&1; then
  ...
  echo "  claude plugin marketplace add hihidev/claude-octopus"
  echo "  claude plugin install claude-octopus@hihidev-plugins --scope user"
...
claude plugin marketplace add hihidev/claude-octopus >/dev/null 2>&1 || true
claude plugin marketplace update hihidev-plugins >/dev/null 2>&1 || true

# Install/enable/update (idempotent).
claude plugin install claude-octopus@hihidev-plugins --scope user >/dev/null 2>&1 || true
...
echo "- Verify install: claude plugin list | grep claude-octopus"
echo ""
